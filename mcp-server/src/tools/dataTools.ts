...
// 분석 결과 저장 도구
export const saveAnalysisResultTool: Tool = {
...
export async function handleSaveAnalysisResult(args: any) {
  try {
...
export async function handleGetAnalysisResult(args: any) {
  try {
...
// 최근 분석 결과 조회 도구
export const getRecentAnalysesTool: Tool = {
...
export async function handleGetRecentAnalyses(args: any) {
  try {
    const { limit = 10, analysisType, riskLevel } = args;

    console.log(
      `최근 분석 결과 조회 시작: limit=${limit}, type=${analysisType}, risk=${riskLevel}`
    );
    ...
    const results = await AnalysisResult.find(query)
      ...
      .select(
        "id timestamp from subject risk.level risk.score analysisType createdAt updatedAt"
      );
    ...
    const statistics = {
      ...
      averageRiskScore:
        results.reduce((sum, r) => sum + (r.risk?.score || 0), 0) /
          results.length || 0,
    };
...
// 분석 보고서 내보내기 도구
export const exportAnalysisReportTool: Tool = {
  name: "mcp_export_analysis_report",
  description:
    "분석 결과를 보고서 형태로 내보냅니다 (JSON 또는 Markdown 형식).",
  inputSchema: {
    type: "object",
    properties: {
      analysisId: {
...
export async function handleExportAnalysisReport(args: any) {
  try {
...
// 분석 통계 조회 도구
export const getAnalysisStatisticsTool: Tool = {
...
export async function handleGetAnalysisStatistics(args: any) {
  try {
          ...
          text: JSON.stringify(
            {
              ...
              statistics: {
                ...
                analysisTypeDistribution: Object.fromEntries(
                  analysisTypeStats.map((stat) => [
                    stat._id || "unknown",
                    stat.count,
                  ])
                ),
...
function generateMarkdownReport(result: any, analysisId: string): string {
...
## 위험도 평가
- **위험 등급**: ${getRiskLevelEmoji(result.risk?.level)} ${
    result.risk?.level || "알 수 없음"
  }
...
## 위험 요소
${
  result.risk?.factors?.map((factor: string) => `- ⚠️ ${factor}`).join("\n") ||
  "- ✅ 발견된 위험 요소가 없습니다"
}
...
${
  result.receivedPaths
    ?.map((path: string, index: number) => `${index + 1}. ${path}`)
    .join("\n") || "수신 경로 정보 없음"
}

### IP 주소 목록
${
  result.ipAddresses?.map((ip: string) => `- ${ip}`).join("\n") ||
  "IP 주소 정보 없음"
}

### 링크 목록
${
  result.links?.map((link: string) => `- ${link}`).join("\n") ||
  "링크 정보 없음"
}
...
function generateStatisticsInsights(data: any): string[] {
  ...
  // 위험도 분포 분석
  const dangerCount =
    riskLevelStats.find((s: any) => s._id === "danger")?.count || 0;
  const dangerRate = ((dangerCount / total) * 100).toFixed(1);

  if (dangerCount > 0) {
    insights.push(
      `⚠️ 전체 분석 중 ${dangerRate}% (${dangerCount}개)가 위험한 이메일로 분류되었습니다.`
    );
  } else {
  ...
  if (dailyStats.length > 1) {
    ...
    if (trend > 0) {
      ...
      insights.push(
        `📉 최근 분석량이 감소하고 있습니다 (${Math.abs(trend)}개 감소).`
      );
    }
  }
