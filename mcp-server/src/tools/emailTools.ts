...
import {
  EmailHeaderAnalysisParams,
  EmailContentAnalysisParams,
  RiskCalculationParams,
  EmailIntentAnalysisParams,
  ComprehensiveEmailAnalysisParams,
  EmailHeaderAnalysisParamsSchema,
  EmailContentAnalysisParamsSchema,
  RiskCalculationParamsSchema,
  EmailIntentAnalysisParamsSchema,
  ComprehensiveEmailAnalysisParamsSchema,
} from "../types.js";
import {
  analyzeEmailHeader,
  ...
  analyzeEmailIntent,
  extractEmailText,
} from "../utils/emailAnalyzer.js";
import { v4 as uuidv4 } from "uuid";

// 이메일 헤더 분석 도구
export const emailAnalyzeHeadersTool: Tool = {
  name: "mcp_email_analyze_headers",
  description:
    "이메일 원문 데이터에서 헤더 정보를 분석하여 SPF, DKIM, DMARC 검증 및 발신 경로를 추적합니다.",
  inputSchema: {
    type: "object",
    properties: {
      rawEmailData: {
...
export async function handleEmailAnalyzeHeaders(args: any) {
  try {
...
// 이메일 콘텐츠 분석 도구
export const emailAnalyzeContentTool: Tool = {
  name: "mcp_email_analyze_content",
  description:
    "이메일 원문 데이터에서 본문 내용, 링크, 첨부파일을 분석하고 추적 픽셀(비콘)을 검사합니다.",
  inputSchema: {
    type: "object",
    properties: {
      rawEmailData: {
...
export async function handleEmailAnalyzeContent(args: any) {
  try {
    ...
    // 본문 및 링크 분석
    let bodyAnalysis: { body: string; links: string[] } = {
      body: "",
      links: [],
    };
    try {
...
// 위험도 계산 도구
export const emailCalculateRiskTool: Tool = {
  name: "mcp_email_calculate_risk",
  description:
    "분석된 이메일 데이터를 바탕으로 종합 위험도 점수(0-100)를 계산합니다.",
  inputSchema: {
    type: "object",
    properties: {
      analysisData: {
        type: "object",
        description:
          "이메일 분석 결과 데이터 (헤더, 콘텐츠, LLM 분석 결과 포함)",
      },
    },
...
export async function handleEmailCalculateRisk(args: any) {
  try {
...
// AI 기반 이메일 의도 분석 도구
export const emailAnalyzeIntentTool: Tool = {
  name: "mcp_email_analyze_intent",
  description:
    "AI를 사용하여 이메일의 의도를 분석합니다 (피싱, 스팸, 정상 등).",
  inputSchema: {
    type: "object",
    properties: {
      emailContent: {
...
export async function handleEmailAnalyzeIntent(args: any) {
  try {
...
// 종합 이메일 분석 도구
export const comprehensiveEmailAnalysisTool: Tool = {
  name: "mcp_comprehensive_email_analysis",
  description:
    "이메일 원문 데이터를 종합적으로 분석합니다. 헤더, 본문, 첨부파일, AI 분석을 모두 포함하며 상세 분석 결과와 위험도 점수, 권장 조치사항을 제공합니다.",
  inputSchema: {
    type: "object",
    properties: {
      rawEmailData: {
...
export async function handleComprehensiveEmailAnalysis(args: any) {
  try {
...
// 헬퍼 함수들
function generateRecommendations(riskAnalysis: any): string[] {
  const recommendations: string[] = [];

  if (riskAnalysis.level === "danger") {
    recommendations.push(
      "⚠️ 이메일을 즉시 삭제하고 링크나 첨부파일을 클릭하지 마세요"
    );
    recommendations.push(
      "📧 발신자에게 직접 연락하여 이메일 진위를 확인하세요"
    );
    recommendations.push("🔒 계정 비밀번호를 변경하고 2FA를 활성화하세요");
  } else if (riskAnalysis.level === "suspicious") {
    recommendations.push(
      "🔍 이메일 내용을 신중히 검토하고 의심스러운 부분이 있는지 확인하세요"
    );
...
function generateDetailedRecommendations(analysisResult: any): any {
  ...
  const { risk, spf, dkim, dmarc, links, beacons, llmAnalysis } =
    analysisResult;
  ...
  if (spf !== "pass" || dkim !== "pass" || dmarc !== "pass") {
    recommendations.preventive.push(
      "발신자 인증이 실패한 이메일은 주의깊게 검토하세요"
    );
  }

  if (beacons && beacons.length > 0) {
    recommendations.preventive.push(
      "추적 픽셀이 감지되었습니다. 이메일 미리보기를 비활성화하세요"
    );
  }
...
function getThreatLevel(category: string, confidence: string): string {
