...
import { IPAnalysisParamsSchema, DomainAnalysisParamsSchema, VirusTotalCheckParamsSchema, PortScanParamsSchema, NetworkThreatAnalysisParamsSchema, } from "../types.js";
// IP 분석 도구
export const analyzeIpTool = {
    name: "mcp_analyze_ip",
...
export async function handleAnalyzeIp(args) {
    try {
        ...
        const flagEmoji = geoipData.flag && geoipData.flag.emoji ? geoipData.flag.emoji : "🏳️";
        // Flask 서버에서 포트 스캔 정보 요청
        let portScanInfo = null;
        const flaskBaseUrl = process.env.FLASK_SERVER_URL || "http://localhost:5001";
        try {
            console.log(`포트 스캔 시작: ${ipAddress}`);
            const portScanResponse = await axios.post(`${flaskBaseUrl}/scan`, { ip: ipAddress, port_range: "21-25,80,443,8080-8090", timeout: 10 }, { timeout: 150000 });
            if (portScanResponse.data && portScanResponse.data.success) {
                console.log(`포트 스캔 성공: ${ipAddress}, 소요시간: ${portScanResponse.data.scan_time}`);
...
// 도메인 분석 도구
export const analyzeDomainTool = {
...
export async function handleAnalyzeDomain(args) {
    try {
            ...
            content: [
                {
                    ...
                    text: JSON.stringify({
                        ...
                        summary: {
                            ...
                            recommendation: ipAnalysis.summary?.recommendation || "추가 조사 필요",
                        },
                        timestamp: new Date().toISOString(),
                    }, null, 2),
                },
...
// VirusTotal 검사 도구
export const virusTotalCheckTool = {
...
export async function handleVirusTotalCheck(args) {
    try {
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            success: false,
                            error: "VirusTotal API 키가 설정되지 않았습니다. 환경 변수를 확인하세요.",
                            timestamp: new Date().toISOString(),
                        }, null, 2),
                    },
                ],
        ...
        let response;
        if (type === "ip") {
            response = await axios.get(`https://www.virustotal.com/api/v3/ip_addresses/${target}`, {
                headers: {
            ...
            const vtResult = {
                ...
                detection_ratio: `${stats.malicious}/${Object.values(stats).reduce((sum, val) => sum + val, 0)}`,
            ...
            response = await axios.get(`https://www.virustotal.com/api/v3/domains/${target}`, {
                headers: {
                    "x-apikey": VIRUSTOTAL_API_KEY,
                },
            });
            ...
            // URL 제출 및 분석
            const submitResponse = await axios.post("https://www.virustotal.com/api/v3/urls", new URLSearchParams({ url: target }).toString(), {
                headers: {
                    "x-apikey": VIRUSTOTAL_API_KEY,
            ...
            // 분석 결과 가져오기
            response = await axios.get(`https://www.virustotal.com/api/v3/analyses/${analysisId}`, {
                headers: {
                    "x-apikey": VIRUSTOTAL_API_KEY,
                },
            });
            ...
            const vtResult = {
                ...
                virustotal_url: `https://www.virustotal.com/gui/url/${encodeURIComponent(target)}`,
            };
            console.log(`VirusTotal 검사 완료: ${target} - ${vtResult.threat}`);
            return {
                content: [
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            ...
                            analysis: {
                                ...
                                detection_ratio: "0/0",
                                virustotal_url: `https://www.virustotal.com/gui/search/${encodeURIComponent(args.target)}`,
                            },
                            summary: {
                                threat: "unknown",
                                detectionRatio: "0/0",
...
export const portScanTool = {
    name: "mcp_port_scan",
    description: "nmap을 이용해 지정된 IP 주소의 포트 스캔 및 배너 그래빙을 수행합니다.",
    inputSchema: {
        type: "object",
        properties: {
            ipAddress: {
...
export async function handlePortScan(args) {
    try {
        ...
        try {
            ...
            const portScanResponse = await axios.post(`${flaskBaseUrl}/scan`, { ip: ipAddress, port_range: portRange, timeout: 10 }, { timeout: 180000 } // 3분 타임아웃
            );
            const scanDuration = Date.now() - startTime;
            if (portScanResponse.data && portScanResponse.data.success) {
                console.log(`포트 스캔 성공: ${ipAddress}, 소요시간: ${scanDuration}ms`);
...
// 네트워크 위협 분석 도구
export const networkThreatAnalysisTool = {
    name: "mcp_network_threat_analysis",
    description: "여러 IP/도메인에 대한 일괄 위험도 분석과 상호 연관성 분석을 수행합니다.",
    inputSchema: {
        type: "object",
        properties: {
            targets: {
...
export async function handleNetworkThreatAnalysis(args) {
    try {
        ...
        // 권장사항 생성
        summary.recommendations = generateNetworkThreatRecommendations(summary, results);
...
// 헬퍼 함수들
function assessIpThreatLevel(geoipData, portScanInfo) {
    let threatScore = 0;
    // 지리적 위치 기반 위험도
...
function getThreatRecommendation(threatLevel) {
    switch (threatLevel) {
        case "매우 높음":
...
function getVirusTotalRecommendation(threat) {
    switch (threat) {
        case "malicious":
...
function analyzeScanResults(scanResult) {
    const analysis = {
        riskLevel: "낮음",
        findings: [],
        recommendations: [],
...
function generateNetworkThreatRecommendations(summary, results) {
    const recommendations = [];
    if (summary.malicious_count > 0) {
        recommendations.push(`${summary.malicious_count}개의 고위험 대상이 발견되었습니다. 즉시 차단 조치를 취하세요.`);
    }
    if (summary.suspicious_count > 0) {
        recommendations.push(`${summary.suspicious_count}개의 의심스러운 대상이 있습니다. 지속적인 모니터링이 필요합니다.`);
    }
    if (summary.error_count > 0) {
        recommendations.push(`${summary.error_count}개 대상의 분석이 실패했습니다. 다른 도구로 재분석하세요.`);
    }
    ...
    if (recommendations.length === 0) {
        recommendations.push("현재 발견된 위협은 없지만 정기적인 모니터링을 계속하세요.");
    }
