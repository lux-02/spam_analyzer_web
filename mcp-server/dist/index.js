    ...
    setupMCPHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                const errorMessage = `도구 실행 중 오류 발생: ${error instanceof Error ? error.message : "알 수 없는 오류"}`;
                ...
                for (const request of requests) {
                    ...
                    try {
                        if (tool in allToolHandlers) {
                            const handler = allToolHandlers[tool];
                            const result = await handler(args);
                            const responseContent = result.content[0].text;
                            const responseData = JSON.parse(responseContent);
                            results.push({
    ...
    async start() {
        ...
        if (mode === "HTTP" && this.httpServer) {
            ...
            this.httpServer.listen(port, host, () => {
                ...
                console.log("\n사용 예시:");
                console.log(`curl -X POST http://${host}:${port}/tools/mcp_email_analyze_headers \\`);
