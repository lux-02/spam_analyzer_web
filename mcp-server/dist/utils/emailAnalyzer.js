...
export function decodeBase64HeaderIfNeeded(value) {
...
export const decodeMIMEHeader = (encodedHeader) => {
    if (!encodedHeader || typeof encodedHeader !== "string")
...
export const analyzeEmailHeader = (emailData) => {
    const ipRegex = /\b(?:\d{1,3}\.){3}\d{1,3}\b/;
    const domainRegex = /@([a-zA-Z0-9.-]+\.[a-zA-Z]{2,6})/;
    const hostRegex = /\((.*?)\[/;
    const dateRegex = /;\s*([A-Za-z]{3},\s+\d{1,2}\s+[A-Za-z]{3}\s+\d{4}\s+\d{1,2}:\d{1,2}:\d{1,2}\s+[-+]\d{4}|\d{1,2}\s+[A-Za-z]{3}\s+\d{4}\s+\d{1,2}:\d{1,2}:\d{1,2}\s+[-+]\d{4})/;
    const ipAddresses = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith("Received:")) {
            ...
            i = j - 1;
            const domainMatches = currentReceivedBlock.fullText.match(new RegExp(domainRegex.source, "g"));
            if (domainMatches) {
...
export function parseEmailBodyAndLinks(rawEmailData) {
    const contentTypeRegex = /Content-Type:\s*text\/(plain|html);\s*charset=["']?([^"';\s]+)["']?/gi;
    const transferEncodingRegex = /Content-Transfer-Encoding:\s*(quoted-printable|base64)/i;
    let charset = "utf-8";
    let body = "";
    let links = [];
...
export const calculateRiskScore = (emailData) => {
    let riskScore = 100;
    const riskFactors = [];
    // SPF 검사
    ...
    if (emailData.llmAnalysis) {
        const { category, confidence, riskScore: llmRiskScore, } = emailData.llmAnalysis;
        if (["비밀번호 변경 요청", "송장/청구서 위장", "로그인 시도 알림"].includes(category)) {
            if (confidence === "High") {
...
export const checkBeaconImages = (htmlContent) => {
    if (!htmlContent || typeof htmlContent !== "string") {
        return [];
    }
    try {
        const beaconPattern = /<img[^>]+(width=['"]?1['"]?[^>]+height=['"]?1['"]?|style=['"]?[^"']*display\s*:\s*none[^"']*['"]?)[^>]*>/gi;
        const beacons = [];
        let match;
...
export const extractEmailText = (emailContent) => {
    try {
        let content = emailContent;
        if (Base64.isValid(emailContent) &&
...
export const analyzeEmailIntent = async (emailContent) => {
    try {
        const emailText = extractEmailText(emailContent);
        const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
        const GEMINI_API_URL = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent";
        if (!GEMINI_API_KEY) {
            console.error("Gemini API 키가 설정되지 않았습니다. 로컬 대체 분석을 실행합니다.");
...
const performLocalFallbackAnalysis = (emailText) => {
    ...
    if (lowerEmailText.includes("비밀번호") &&
    ...
    else if (lowerEmailText.includes("로그인") &&
    ...
    else if (lowerEmailText.includes("청구서") ||
    ...
    else if (lowerEmailText.includes("초대") ||
    ...
    else if (lowerEmailText.includes("할인") ||
...
const getAnalysisMessage = (category, confidence) => {
    const confidenceMap = {
        High: "높습니다",
        Medium: "중간 수준입니다",
        Low: "낮습니다",
    ...
    return `이 메일은 ${categoryMap[category] || category} 가능성이 ${confidenceMap[confidence] || confidence}.`;
};
//# sourceMappingURL=emailAnalyzer.js.map